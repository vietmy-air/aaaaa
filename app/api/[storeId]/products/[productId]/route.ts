...
import {  Product, Size } from "@/type-db";
...
export const DELETE = async (req: Request, {params} : {params : {storeId: string, productId: string}}) => { 
    try {
        ...
        return NextResponse.json({msg: "Product and associated images delete successly"})
...
export const GET = async ( 
    ...
    try {     
        ...
        if(!params.productId) { 
            return new NextResponse("Product is missing", {status: 400})
        }  
        const product = (
